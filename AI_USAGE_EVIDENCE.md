...
모델별 역할, API 호출 위치, 프롬프트 원칙과 출력 계약은 이 문서의 앞 절에, 자동·운영 검증 결과는 6절과 [`VALIDATION.md`](VALIDATION.md)에 정리했다. 서비스 전체 사용 방법과 로컬 실행 절차는 [`README.md`](README.md)를 따른다.
...
팀은 ChatGPT와 Codex를 다음 단계에서 실제 개발 보조 도구로 활용했다.

| 개발 단계 | 활용 방식 | 사람의 검증 |
|---|---|---|
| 기획 | 인간다움의 문제 정의, 회상 우선 사용자 흐름, 동네 기억 보존 정책 구체화 | 팀 회의를 통해 구현 범위와 표현 확정 |
| 구현 | FastAPI·Supabase·Upstage 연동 초안, 프런트엔드 모듈화와 리팩터링 보조 | 기존 API 계약과 DB 스키마를 대조해 코드 리뷰 |
| 디버깅 | Python 환경, 환경변수, DB Pooler, AI 요청, UI 상태와 중복 카드 오류 분석 | 실제 오류 로그와 재현 절차로 원인 확인 |
| 안전 설계 | 비식별화·삭제·중복 기여·원본 공개 전 추측 처리의 예외 상황 검토 | 개인정보 정책과 테스트 사례로 검증 |
| 검증 | 자동 테스트 작성 보조, Railway 브라우저 흐름과 운영 API 통합 점검 | 48개 자동 테스트 및 실제 외부 API 호출 확인 |
| 협업 | 변경 범위 확인, 커밋·PR 설명, README와 제출 증빙 정리 | 팀원이 최종 diff와 PR을 검토한 뒤 병합 |

개발 중에는 브라우저 점검 기능으로 Railway 배포 UI와 실제 사용자 흐름을 확인하고, GitHub 연동 기능으로 커밋·PR 상태와 default 브랜치 반영 여부를 검증했다. AI가 생성하거나 수정한 결과는 자동으로 채택하지 않았으며, 팀원이 코드·정책을 검토하고 테스트를 통과한 변경만 저장소에 반영했다.

공개 가능한 AI 관련 실행 설정은 `.claude/launch.json`에 포함했다. 실제로 사용하지 않은 `.agents`, `.omc` 등의 설정은 증빙을 위해 임의로 만들지 않았으며, API 키와 비밀번호 등 민감정보는 저장소에서 제외했다.
