...
골목·계단·평상·화분·빨랫줄·생활 소리·일반적인 가게 유형처럼 개인을 특정하지 않는
지역의 공간·생활·감각 정보는 지우지 말고 safe_fragments에 구체적으로 유지하세요.
서로 다른 기여자 2명 이상에게 의미상 반복된 장소의 활동·분위기·감각을 common_themes에 포함하세요.
표현이 정확히 같지 않아도 '작은 가게와 사라진 가게', '화분과 주민의 일상'처럼
같은 지역 생활상을 가리키면 하나의 공통 주제로 묶을 수 있습니다.
개인별 사건을 재구성할 수 있는 문장은 safe_fragments에 넣지 않되,
각 조각에서 안전하게 공개할 수 있는 지역 생활 장면은 최소 하나 이상 남기세요.
...
단, 골목·계단·평상·화분·빨랫줄·라디오나 식사 준비 같은 생활 소리·문방구나 작은 가게처럼
개인을 특정하지 않는 지역의 공간·생활·감각 정보는 삭제하지 말고 구체적으로 유지하세요.
장소 태그 '{place_tag}'와 함께 지역의 모습이 떠오를 정도로 한두 문장으로 정리하고,
원문에 안전한 지역 생활 세부 정보가 있다면 최소 두 가지를 남기세요.
...
추상적인 '함께한 시간'이나 '장소의 분위기'만 반복하지 말고, 제공된 조각에 있는
골목·생활 소리·작은 가게·화분 같은 안전한 지역 생활 장면을 구체적으로 연결하세요.
story는 3~5문장으로 작성하여 장소의 모습, 반복된 생활 장면, 시간이 지나며 보존할 가치가 있는
지역적 의미가 자연스럽게 이어지게 하세요. reflection은 공동체 기억의 의미를 한 문장으로 씁니다.
