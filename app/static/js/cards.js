...
export async function loadCards() {
  ...
  try {
    ...
    const articles = await Promise.all(state.cards.map(async (card) => {
      ...
      return `<article class="memory-card"><div class="meta">${formatDate(card.memory_date)}${card.place_label ? ` · ${escapeHtml(card.place_label)}` : ''} ${shareLabel ? `<span class="shared-chip ${card.town_share_status}">${escapeHtml(shareLabel)}</span>` : ''}</div><h3>${escapeHtml(card.card_title)}</h3>${updateNotice}${await imageMarkup(card)}${imageTools(card)}<p>${escapeHtml(card.story)}</p><p><b>${escapeHtml(card.reflection)}</b></p>${history ? `<details class="recall-history"><summary>${card.recall_history.length}번의 회상 과정 보기</summary><ol>${history}</ol></details>` : ''}<div class="card-actions"><input id="place-${card.id}" list="placeTagsList" value="${escapeHtml(card.place_tag || '')}" placeholder="공유 지역 검색" aria-label="공유 장소" autocomplete="off"><button class="secondary share-town" data-card-id="${card.id}">${card.shared_to_town ? '공유 장소 변경' : '동네에 익명 공유'}</button>${card.shared_to_town ? `<button class="ghost unshare-town" data-card-id="${card.id}">공유 취소</button>` : ''}<button class="ghost archive-card" data-card-id="${card.id}" data-archived="${!showingArchived}">${showingArchived ? '다시 보관함에 놓기' : '카드 숨기기'}</button><button class="danger-outline delete-card-memory" data-memory-id="${card.memory_id}">개인 기억 삭제</button></div><p class="action-help">카드 숨기기는 복구할 수 있고, 개인 기억 삭제는 원본과 회상을 함께 지워 복구할 수 없습니다.</p></article>`;
    }));
